...
class DataSave():
    ...
    def create_table(self,conn,curs,table_name):
        ...
        try:
            ...
            first_data_timestamp TIMESTAMP NOT NULL,
            last_data_timestamp TIMESTAMP NOT NULL,
            clock_offsent INTEGER NOT NULL,
            PRIMARY KEY(id))
            ''' % table_name
    ...
    def insert_table(self,conn,curs,table_name):
        ...
        try:
            ...
            '''
            sql = "insert into "+table_name+"(src_device,dst_device,template_name,detect_value,detect_timestmap,first_data_timestamp,last_data_timestamp,clock_offsent) values(%s,%s,%s,%s,%s,%s,%s,%s)"
            print sql
    ...
    def run(self):
        ...
        if self.db_name not in db_list:
            ...
            if result == 1:
                ...
                for table_name in customer_list:
                    if table_name not in tb_list:
                        result_table = self.create_table(conn,curs,table_name)
                    else:
                        pass
        else:
            conn = self.get_conn()
...
def insert_table(conn,curs,table_name,params):
    '''
    向数据库中插入数据
    '''
    ...
    try:
        sql = "insert into "+table_name+"(src_device,dst_device,template_name,detect_value,detect_timestmap,first_data_timestamp,last_data_timestamp,clock_offsent) values(%s,%s,%s,%s,%s,%s,%s,%s)"
        print sql
        result = curs.executemany(sql,params)
        conn.commit()
        return result
    except Exception,e:
        log_message.error("插入数据失败:%s" % str(e))

def check_table(curs,db_name):
    '''
    检查数据库中存在的表
    '''
    tb_list = []
...
if __name__ == '__main__':
    ...
    params_list = [['23','http','5678','890','32','21','18',9],['23','45','http','890','5678','45','89',9],['23','ping','56','56','78','34','89',9],['54','ping','34','54','12','12','89',9]]
    ...
    result_data = insert_table(conn,curs,'customer_cok',params_list)
    print result_data
    b = check_table(curs,db_name)
    print b
