...
class HTTP_with_timeout(httplib.HTTP):
    def __init__(self,host='',port=None, stict=None, timeout=10):
        
        if port==0: port=None
        self._setup(self._connection_class(host, port, stict, timeout=timeout))
...
class TimeoutServerProxy(xmlrpclib.ServerProxy):
    def __init__(self,uri,timeout=10,*l,**kw):
        t=TimeoutTransport(timeout=timeout, use_datetime=kw.get('use_datetime',0))
        xmlrpclib.ServerProxy.__init__(self,uri,t,*l,**kw)
